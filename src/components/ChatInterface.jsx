...
export default function ChatInterface({ auth }) {
  ...
  const [messages, setMessages] = useState([
    { id: 1, role: "agent", text: `Namaskara ${auth.name}! I am the KSP Intelligence Assistant. How can I help you today?`, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) },
  ]);
  ...
  const handleSend = async () => {
    ...
    const userMsg = { id: Date.now(), role: "user", text: input.trim(), time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) };
    setMessages((prev) => [...prev, userMsg]);
    setInput("");
    setLoading(true);
    try {
      const data = await askQuestion(userMsg.text, sessionId, auth.token);
      setMessages((prev) => [...prev, { id: Date.now()+1, role: "agent", text: data.reply, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }]);
    } catch (e) {
      setMessages((prev) => [...prev, { id: Date.now()+1, role: "agent", text: `⚠️ ${e.message}`, time: "" }]);
    ...
    <div className="chat-container">
      <div className="chat-header">
        ...
        <div>
          ...
          <div className="chat-status">● Online · Karnataka State Police</div>
        </div>
      </div>
      ...
      <div className="chat-input-area">
        ...
        <div className="session-id">Session: {sessionId.slice(0,8)} · Secure Channel</div>
      </div>
    </div>
  );
}
